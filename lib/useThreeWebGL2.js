...
export function useDarkScene( tjs, props={} ){
    const pp = Object.assign( { ambient:0x808080, grid:true }, props );

    // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    // LIGHTS
    
    // Main Lighting
    const litKey = new THREE.DirectionalLight( 0xffffff, 1.5 );
    litKey.position.set( 4, 4, 4 );
    tjs.scene.add( litKey );

    // Sky & Ground Lighting
    const litHemi = new THREE.HemisphereLight( 0xb1e1ff, 0xb97a20, 1.0 );
    litHemi.position.set( 0, 50, 0 );
    tjs.scene.add( litHemi );

    // Extra Ligghts
    tjs.scene.add( new THREE.AmbientLight( pp.ambient ) );
    
    const litFill = new THREE.PointLight( 0xffffff, 3, 100 );
    litFill.position.set( -2, 0.6, 2 );
    tjs.scene.add( litFill );

    const litFill2 = new THREE.PointLight( 0xffffff, 5, 50 );
    litFill2.position.set( 0, 2, -4 );
    tjs.scene.add( litFill2 );
