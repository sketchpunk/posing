...
export default class ShapeQuads extends THREE.Mesh{
    ...
    add( pos, color=this._defaultColor, size=this._defaultSize, shape=this._defaultShape ){
        ...
        if( Array.isArray( color ) ) this._col.push(...color);          // Already GLSL encoded
        else                         this._col.push(...glColor(color)); // Numeric Hex, need glsl encoding
    ...
    _updateGeometry(){
        ...
        if(
            ...
            console.log( 'NEED TO REDO GEOMETRY' );
...
function createInstanceGeo( xGeo=null, pos, col, cfg ) {
    // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    const geo = new THREE.InstancedBufferGeometry();
    // geo._maxInstanceCount = Infinity; // USE IF BUGGY
    ...
    geo.setIndex(new THREE.BufferAttribute(xGeo.index.array, 1));
